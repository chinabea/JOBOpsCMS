...
class TicketController extends Controller
{
    ...
    {
        // Get specific users for the filter sidebar
        $userIds = User::where('role', 2)->where('is_approved', true)->get();

        // Apply filters and sorting
        $tickets = $this->applyFiltersAndSort($request);

        if ($request->ajax()) {
            return view('ticket.partials.ticket_list', compact('tickets'))->render();
        }
    ...
    protected function applyFiltersAndSort(Request $request)
    {
        ...
        if ($request->filled('building_number')) {
            $tickets->where('building_number', $request->building_number);
        }
        ...
        if ($request->filled('sort_by')) {
            ...
            $tickets->orderBy($request->sort_by, $sortOrder);
        }
